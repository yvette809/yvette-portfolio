...
export default function BlogPreview() {
  ...
  return (
    <section id="blog" ref={ref} className="py-28 px-[5vw] relative z-[2]">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {recentPosts.map((post, i) => (
          <Link
            key={post.slug}
            href={`/blog/${post.slug}`}
            className="reveal project-card group bg-surface border border-white/[0.07] rounded-xl p-7 transition-all duration-400 hover:-translate-y-1.5 hover:border-mint/30 hover:shadow-[0_20px_60px_rgba(0,0,0,0.5),0_0_0_1px_rgba(95,255,215,0.1)] flex flex-col"
          ...
          >
            ...
            <h3 className="font-head text-xl font-bold text-white mb-3 leading-tight tracking-[-0.01em] group-hover:text-mint transition-colors">
              {post.title}
            </h3>

            <p className="text-sm text-muted leading-[1.7] flex-1 mb-5">
              {post.excerpt}
            </p>
